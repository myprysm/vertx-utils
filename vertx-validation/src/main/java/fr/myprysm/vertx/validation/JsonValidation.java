...
public interface JsonValidation extends Function<JsonObject, ValidationResult> {
     ...
     * @return validation result combinator
     */
    static <T extends Enum<T>> JsonValidation isEnumInRange(String field, T... values) {
        requireNonNull(field);
    ...
    static <T extends Enum<T>> JsonValidation isEnumInRange(String field, String message, T... values) {
        requireNonNull(field);
        requireNonNull(values);
        return isEnum(field, values[0].getDeclaringClass())
                .and(holds(json -> {
                    Object value = json.getValue(field);
                    List<String> enumValues = Arrays.stream(values)
                            .map(Enum::name)
                            .collect(Collectors.toList());
                    if (value instanceof String) {
                        return enumValues.contains(value);
     ...
     * @return validation result combinator
     */
    static JsonValidation nested(String field, Supplier<JsonValidation> supplier) {
        requireNonNull(field);
        requireNonNull(supplier);
        AtomicReference<String> nestedMessage = new AtomicReference<>();
     ...
     * @return the validation
     */
    static JsonValidation holds(Predicate<JsonObject> p, Supplier<String> messageSupplier) {
