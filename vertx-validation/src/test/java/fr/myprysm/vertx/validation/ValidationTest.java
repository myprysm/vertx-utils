...
class ValidationTest implements BaseJsonValidationTest {
    private static final JsonObject JSON = new JsonObject()
            ...
            .put("enum", "SECONDS")
            .put("enumArray", new JsonArray().add("SECONDS").add("MINUTES").add("HOURS"))
            .put("true", true)
            .put("false", false)
            .put("long", 10L)
            .put("array", new JsonArray().add("item1").add("item2"))
            .put("intArray", new JsonArray().add(1).add(2).add(3))
    ...
    void testValidationResultUtils() {
        ...
        assertThat(invalid("test").toException().getMessage()).isEqualTo(new ValidationException(invalid("test")).getMessage());
    ...
    @Test
    @DisplayName("Test nested validations")
    void testNestedValidation() {
        isValid(JSON, nested("object", isLong("number").and(isString("string")).and(isBoolean("boolean"))));
        isInvalid(JSON, nested("intMap", isLong("int1").and(isString("int2"))),
                message("intMap", "is not valid. Nested error is: " + message("int2", "is not a string")));
    }
    ...
    void testObjectsAndMaps() {
        ...
        isValid(JSON, hasPath("map.field1"));
        isInvalid(JSON, hasPath("some.dummy.path"), message("some.dummy.path", "does not exist"));
    }
    ...
    void testStrings() {
        ...
        isValid(JSON, isEnumInRange("enum", TimeUnit.HOURS, TimeUnit.MINUTES, TimeUnit.SECONDS));
        isInvalid(JSON, isEnumInRange("enum", TimeUnit.HOURS, TimeUnit.MINUTES), message("enum", "is not part of enum HOURS,MINUTES"));

        isValid(JSON, isEnumInRange("enumArray", TimeUnit.HOURS, TimeUnit.MINUTES, TimeUnit.SECONDS));
        isInvalid(JSON, isEnumInRange("enumArray", TimeUnit.DAYS, TimeUnit.MILLISECONDS), message("enumArray", "is not part of enum DAYS,MILLISECONDS"));
