 ...
 * <p>
 * Please note that the extraction API is better to use them after having validated
 * your input...
 */
public interface JsonHelpers {
    ...
    static Optional<JsonObject> extractJsonObject(JsonObject json, String path) {
        ...
        Optional<JsonObject> source = Optional.of(json);
    ...
    static Optional<String> extractString(JsonObject json, String path) {
        requireNonNull(json);
        Pair<String, String> field = toPathAndField(path);
        return extractJsonObject(json, field.getKey()).map(sub -> sub.getValue(field.getValue()).toString());
    }
